...
import { createFynappVersion, kernelVersion } from "./template-versions.js";
 ...
 *
 * `{{kernelVersion}}` and `{{createFynappVersion}}` come from create-fynapp's
 * own manifest rather than the caller's config -- a scaffolded app has to be
 * pinned to the kernel these templates were written against and to the release
 * that scaffolded it, and neither is the user's choice to make (FYM-285).
 */
function applyTemplateVars(content: string, config: GeneratorConfig): string {
    return content
        ...
        .replace(/\{\{createFynappVersion\}\}/g, createFynappVersion());
}
